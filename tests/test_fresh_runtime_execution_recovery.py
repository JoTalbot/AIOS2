...
async def test_fresh_runtime_recovers_persisted_execution_and_executes_once(tmp_path):
    ...
    store.save(ExecutionState(execution_id, status="running", goal="resume task", attempt=0,
                              plan=[{"tool": "work", "arguments": {"goal": "resume task"}}]))
    runner = Runner()
    fresh_loop = AutonomousExecutionLoop(runner, Planner(), ReplanningPolicy(max_attempts=2), store=store)
