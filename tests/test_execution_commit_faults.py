...
def test_reconcile_skips_pending_commit_when_fencing_token_is_stale(tmp_path):
    ...
    store = ExecutionStore(str(tmp_path / "executions.json"))
    audit = ExecutionAuditLog(str(tmp_path / "audit.jsonl"))
    state = store.save(ExecutionState("exec-3", status="running", attempt=1, correlation_id="corr-3"))
    ...
    coordinator = ExecutionCommitCoordinator(
        ...
        lease_owner_id="node-a",
        fencing_token=old_lease.fencing_token,
    )
    coordinator._append_journal(
        coordinator._read_journal()[0]
        if coordinator._read_journal()
        else __import__("runtime.execution_commit", fromlist=["ExecutionCommit"]).ExecutionCommit(
            "exec-3:1:completed:corr-3", "exec-3", "running", "completed", 1, {"stale": True}, correlation_id="corr-3"
        )
    )
