...
def test_stale_fencing_token_cannot_reconcile_pending_commit(tmp_path):
    ...
    coordinator._append_journal(
        ExecutionCommit("c1", "e1", "running", "completed", 1, {"stale": True}, correlation_id="c1")
    )
...
def test_commit_records_intent_but_does_not_apply_after_fencing_loss(tmp_path, monkeypatch):
    ...
    calls = {"count": 0}
    original = leases.is_owner

    def fenced(*args, **kwargs):
        calls["count"] += 1
        return original(*args, **kwargs) if calls["count"] == 1 else False

    monkeypatch.setattr(leases, "is_owner", fenced)
