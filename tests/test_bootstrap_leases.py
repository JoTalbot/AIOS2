...
@pytest.mark.asyncio
async def test_bootstrap_checks_fencing_after_async_resume():
    class FencedAfterResumeLease:
        ttl_seconds = 30
        checks = 0

        def acquire(self, execution_id, owner):
            return type("Lease", (), {"fencing_token": 11})()
    ...
    class Store:
        def resumable(self):
            return [ExecutionState("e2", status="running")]
    ...
    report = await RuntimeBootstrap(
        store=Store(),
        lease_store=FencedAfterResumeLease(),
        owner_id="runtime-a",
