...
def create_app(*, recovery_service: Optional[RecoveryOperatorService] = None, operator_validator: Optional[Callable[[Request], Optional[SecurityContext]]] = None, readiness_check: Optional[Callable[[], bool]] = None, auth_config: Optional[ControlPlaneAuthConfig] = None):
    ...
    service = recovery_service or RecoveryOperatorService(); config = auth_config
    if config is None and operator_validator is None: config = ControlPlaneAuthConfig.from_env()
    def authorize(request: Request):
        context = operator_validator(request) if operator_validator is not None else authenticate(request, config)
        if not context: raise HTTPException(status_code=403, detail="operator authorization required")
        request.state.operator_context = context; return context
    @app.get("/health", tags=["system"])
    async def health(): return {"status":"ok","system":"AIOS"}
    @app.get("/ready", tags=["system"])
    async def ready():
        if readiness_check is not None and not readiness_check(): raise HTTPException(status_code=503, detail="AIOS dependencies are not ready")
        return {"status":"ready","system":"AIOS"}
    app.include_router(build_recovery_router(service, authorize_operator=authorize)); return app
