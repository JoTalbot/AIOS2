...
class RuntimeBootstrap:
    ...
    async def _run_with_heartbeat(self, result, execution_id, fencing_token):
        ...
        A heartbeat task used to be cancelled in ``finally`` without its
        exception being observed.  A lost/fenced lease could therefore leave
        recovery reported as successful.  For async recovery, race the resume
        task against the heartbeat and propagate heartbeat failure first.
        """
            ...
            if decision.action in {
                ...
                self.recovery_queue.enqueue(
                    RecoveryQueueItem(
                        ...
                        decision.reason,
                        state.attempt,
                        getattr(state, "correlation_id", None),
                    )
                )
                quarantined += decision.action is RecoveryAction.QUARANTINE
                manual_review += decision.action is RecoveryAction.MANUAL_REVIEW
                continue
    ...
    async def recover_with_loop(self, loop, agent, context=None):
        return await self.recover_pending(
            lambda state: loop.resume(state.execution_id, agent, context=context)
        )
