...
class ExecutionAuditEvent:
    ...
    def with_identity(self):
        if self.event_id:
            return self
        payload = f"{self.execution_id}|{self.from_status}|{self.to_status}|{self.attempt}|{self.reason}|{self.timestamp}|{self.correlation_id}|{self.version}"
...
class ExecutionAuditLog:
    ...
    def events(self, execution_id: Optional[str] = None):
        ...
        for line in self.path.read_text(encoding="utf-8").splitlines():
            if line.strip():
                raw = json.loads(line)
                raw.setdefault("version", None)
                event = ExecutionAuditEvent(**raw)
...
class ExecutionAudit:
    """Tool/event audit with optional durable lifecycle compatibility API."""
    ...
    def record_transition(self, execution_id: str, agent_id: str, from_status: str, to_status: str, attempt: int = 0, reason: Optional[str] = None):
        event = ExecutionAuditEvent(execution_id, from_status, to_status, attempt, reason)
        if self.path:
            event = event.with_identity()
            with self.path.open("a", encoding="utf-8") as handle:
                handle.write(json.dumps(asdict(event), ensure_ascii=False) + "\n")
