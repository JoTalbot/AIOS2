...
class ExecutionLeaseStore:
    def __init__(self, path: str = "data/execution_leases.json", ttl_seconds: int = 60):
        self.path = Path(path); self.path.parent.mkdir(parents=True, exist_ok=True); self.ttl_seconds = max(1, ttl_seconds)
        if not self.path.exists(): self._write({})
    def _read(self):
        try: return json.loads(self.path.read_text(encoding="utf-8"))
        except (FileNotFoundError, json.JSONDecodeError): return {}
    def _write(self, data):
        tmp = self.path.with_suffix(self.path.suffix + ".tmp"); tmp.write_text(json.dumps(data, indent=2), encoding="utf-8"); tmp.replace(self.path)
    ...
    def is_owner(self, execution_id: str, owner_id: str) -> bool:
        current = self._read().get(execution_id); return bool(current and current["owner_id"] == owner_id and datetime.fromisoformat(current["expires_at"]) > datetime.now(timezone.utc))
    def _store(self, execution_id, owner_id, data, now):
        lease = ExecutionLease(execution_id, owner_id, (now + timedelta(seconds=self.ttl_seconds)).isoformat()); data[execution_id] = {"owner_id": lease.owner_id, "expires_at": lease.expires_at}; self._write(data); return lease
    def release(self, execution_id: str, owner_id: str) -> bool:
        data = self._read(); current = data.get(execution_id)
        if not current or current["owner_id"] != owner_id: return False
        del data[execution_id]; self._write(data); return True
