...
class ExecutionLeaseStore:
    ...
    def _read(self):
        ...
        for execution_id, lease in data.items():
            ...
            if not isinstance(lease.get("fencing_token", 0), int) or lease.get("fencing_token", 0) < 1:
                raise LeaseCorruptionError("execution lease entry has invalid fencing token")
        return data
    ...
    def acquire(self, execution_id: str, owner_id: str) -> Optional[ExecutionLease]:
        if not execution_id or not owner_id:
            raise ValueError("execution_id and owner_id are required")
    ...
    def is_owner(self, execution_id: str, owner_id: str, fencing_token: Optional[int] = None) -> bool:
        ...
        return bool(
            ...
            and (fencing_token is None or current["fencing_token"] == fencing_token)
    ...
    def release(self, execution_id: str, owner_id: str, fencing_token: Optional[int] = None) -> bool:
        data = self._read()
        current = data.get(execution_id)
        if not current or current["owner_id"] != owner_id or (fencing_token is not None and current["fencing_token"] != fencing_token):
            return False
