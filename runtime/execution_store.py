...
def _restore_result(value: Any) -> Any:
    if isinstance(value, list):
        return [_restore_result(item) for item in value]
    if isinstance(value, dict):
        tool_keys = {"call_id", "tool", "ok", "value", "error", "retryable"}
        if tool_keys.issubset(value):
            return ToolResult(
                call_id=value["call_id"], tool=value["tool"], ok=bool(value["ok"]),
                value=value.get("value"), error=value.get("error"), retryable=bool(value.get("retryable", False))
            )
        return {key: _restore_result(item) for key, item in value.items()}
    return value
...
class ExecutionStore:
    ...
    def __init__(self, path: str = "data/executions.json", state_machine: Optional[ExecutionStateMachine] = None, audit_log: Optional[ExecutionAuditLog] = None):
        self.path = Path(path); self.lock_path = self.path.with_suffix(self.path.suffix + ".lock"); self.path.parent.mkdir(parents=True, exist_ok=True); self.state_machine = state_machine or ExecutionStateMachine(); self.audit_log = audit_log
    ...
    def _write(self, data: Dict[str, Any]) -> None:
        tmp = self.path.with_suffix(self.path.suffix + ".tmp"); tmp.write_text(json.dumps(data, ensure_ascii=False, default=str, indent=2), encoding="utf-8"); tmp.replace(self.path)
    def create(self, execution_id: str, metadata: Optional[Dict[str, Any]] = None, **fields) -> ExecutionState:
        with self._lock():
            ...
            values = dict(fields); values.update(metadata or {}); allowed = {"status", "goal", "attempt", "plan", "result", "error", "correlation_id", "version", "fencing_token"}
            state = ExecutionState(execution_id=execution_id, **{k: v for k, v in values.items() if k in allowed}); state.updated_at = datetime.now(timezone.utc).isoformat(); data[execution_id] = asdict(state); self._write(data)
        return state
    def save(self, state: ExecutionState, *, _audit: bool = True, expected_version: Optional[int] = None, fencing_token: Optional[int] = None) -> ExecutionState:
        with self._lock():
            ...
            if previous:
                actual = int(previous.get("version", 0)); expected_version = actual if expected_version is None else expected_version
        ...
        if _audit and previous and self.audit_log and old_status != state.status: self.audit_log.append(ExecutionAuditEvent(state.execution_id, old_status or "new", state.status, state.attempt, state.error, correlation_id=state.correlation_id, version=state.version))
        return state
    def transition(self, execution_id: str, status: str, *, _audit: bool = True, expected_version: Optional[int] = None, fencing_token: Optional[int] = None, **updates) -> ExecutionState:
        ...
        if _audit and self.audit_log and old_status != status: self.audit_log.append(ExecutionAuditEvent(state.execution_id, old_status or "new", status, state.attempt, state.error, correlation_id=state.correlation_id, version=state.version))
        return state
    def get(self, execution_id: str) -> Optional[ExecutionState]:
        with self._lock(): raw = self._read().get(execution_id)
        if not raw: return None
        state = ExecutionState(**raw); state.result = _restore_result(state.result); return state
    def resumable(self):
