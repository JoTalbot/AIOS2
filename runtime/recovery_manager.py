...
class RecoveryManager:
        ...
        for state in self.pending():
            ...
            try:
                ...
                if resume is not None:
                    result = resume(state.execution_id, agent, context=context)
                else:
                    result = loop.run(state.goal, agent, context=context, execution_context=None)
                ...
                if lease is not None: self.lease_store.release(state.execution_id, self.owner_id, lease.fencing_token)
    ...
    def mark_failed(self, state: ExecutionState, error: BaseException, *, lease=None):
        if lease is not None and (self.lease_store is None or not self.lease_store.is_owner(state.execution_id, self.owner_id, lease.fencing_token)): return False
