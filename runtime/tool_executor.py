...
class ToolExecutor:
    ...
    async def execute(self, call: ToolCall, context: ToolExecutionContext, execution_context: ExecutionContext | None = None) -> ToolResult:
        ...
        async with lock:
            ...
            try:
                ...
                if result.ok:
                    if self.idempotency_store:
                        stored = self.idempotency_store.put_if_absent(StoredToolResult(key, call.call_id, call.tool, True, result.value)); result = ToolResult(call.call_id, call.tool, stored.ok, stored.value, stored.error, False, stored.idempotency_key)
                ...
                elif self.intent_store: self.intent_store.release_claim(key, claim_owner, claim_token, "ambiguous")
                return result
            except asyncio.CancelledError:
                # Keep the durable claim in executing state. Recovery, not
                # cancellation cleanup, decides whether the side effect can be reconciled.
                raise
    ...
    async def reconcile_intent(self, intent: ToolIntent, resolver):
        ...
        if self.idempotency_store:
            ...
            if stored:
                if self.intent_store and intent.state == "ambiguous": self.intent_store.mark(intent.idempotency_key, "completed")
    ...
    async def _execute_once(self, call: ToolCall, context: ToolExecutionContext, execution_context: ExecutionContext | None = None) -> ToolResult:
        ...
        try:
            ...
            await self._publish(TOOL_FAILED, ctx, {"tool": call.tool, "call_id": call.call_id, "error": str(exc), "retryable": retryable, "idempotency_key": call.idempotency_key}); return result
