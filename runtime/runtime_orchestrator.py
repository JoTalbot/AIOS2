...
class RuntimeOrchestrator:
    def __init__(self, executor, planner, *, owner_id="aios-runtime", store=None, lease_store=None, policy=None, event_bus=None, audit_log=None, commit_coordinator=None):
        ...
        self.recovery_manager = RecoveryManager(self.store, commit_coordinator=self.commit_coordinator)
        self.bootstrap = RuntimeBootstrap(store=self.store, recovery_manager=self.recovery_manager, lease_store=self.lease_store, owner_id=owner_id, commit_coordinator=self.commit_coordinator)
        self.loop = build_execution_loop(executor, planner, owner_id=owner_id, store=self.store, lease_store=self.lease_store, policy=policy, event_bus=event_bus, audit_log=self.audit_log, commit_coordinator=self.commit_coordinator)
